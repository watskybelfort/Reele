...
/**
 * La marca: el mismo triangulo del SVG de la barra de titulo.
 *
 * Sounde usa cinco barras porque su asunto es el sonido; aqui el asunto es
 * la imagen, y el triangulo de reproducir es el unico simbolo que se lee al
 * instante a 16 pixeles sin parecerse a nada mas de la bandeja.
 */
const LOGO = () => [triangulo(0.3, 0.16, 0.3, 0.84, 0.82, 0.5)];
...
/**
 * Violeta, no el azul de Sounde.
 *
 * No es capricho: las dos aplicaciones acaban una al lado de la otra en la
 * barra de tareas, y con el mismo color y la misma silueta de cuadrado
 * redondeado habria que leer el tooltip para saber cual es cual.
 */
const ACENTO = [167, 139, 250];
/** El mismo violeta, hundido, para el degradado del icono grande. */
const ACENTO_HONDO = [109, 79, 208];

/**
 * El triangulo del icono de la aplicacion.
 *
 * Mas centrado y algo mas pequeno que el de la bandeja: aqui vive dentro de
 * un cuadrado con margen, no suelto sobre el fondo. El vertice se lleva un
 * poco a la derecha del centro geometrico porque un triangulo centrado por
 * su caja se ve desplazado hacia la izquierda — el area esta toda a ese lado.
 */
const LOGO_GORDO = () => [triangulo(0.36, 0.27, 0.36, 0.73, 0.72, 0.5)];

/**
 * Por debajo de aqui el triangulo se dibuja con los bordes cuadrados a la
 * rejilla de pixeles.
 */
const MENUDO = 24;

/**
 * El mismo triangulo, pero con los limites en pixeles enteros.
 *
 * A 16 pixeles el del dibujo grande empieza en 5,76 y termina en 11,52: los
 * dos lados caen a caballo entre dos pixeles, el suavizado los reparte en
 * medias tintas, y lo que se ve es una mancha gris con forma vaga. Cuadrando
 * la base y la punta a la rejilla, el triangulo tiene bordes de verdad y solo
 * suaviza la diagonal, que es donde el suavizado ayuda.
 *
...
function logoMenudo(tam) {
  const izquierda = Math.round(tam * 0.36) / tam;
  const derecha = Math.round(tam * 0.72) / tam;
  const arriba = Math.round(tam * 0.27) / tam;
  const abajo = Math.round(tam * 0.73) / tam;
  return [triangulo(izquierda, arriba, izquierda, abajo, derecha, (arriba + abajo) / 2)];
}
...
/** La marca de Reele en el color de acento, para la bandeja del sistema. */
function marca() {
  return conRepresentaciones((tam) => pintar(tam, { formas: LOGO(), color: ACENTO }));
}

/**
 * El icono de la aplicacion, en color y a cualquier tamano.
 *
 * Triangulo blanco sobre cuadrado violeta, no al reves. Un icono oscuro
 * desaparece en una barra de tareas oscura, que es la de casi todo el mundo.
 * El degradado es leve: a 16 no se ve, y a 256 evita que parezca un
 * rectangulo de pintura plana.
 *
 * De 24 para abajo el triangulo se cuadra con la rejilla; ver logoMenudo.
 */
 ...
 * un .ico multitamano se queda con la representacion de 256 y todo lo demas
 * sale de reescalarla; pasandole la ruta, Windows abre el contenedor y coge de
 * dentro el tamano exacto que pide cada sitio, que es para lo que se generan
...
function iconoVentana() {
  ...
  // 32 de base y 64 para pantallas al 200%. Los tamanos grandes se dejan
 ...
 * color: es un cuadrado redondeado con el triangulo encima. `color` puede ser
